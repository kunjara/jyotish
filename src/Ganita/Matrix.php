...
class Matrix
{
    ...
    public function addMatrix(Matrix $matrix)
    {
        ...
        if ($dimensionsThis != $dimensionsMatrix) {
            throw new Exception\RuntimeException("Dimensions of matrix is not equal.");
        } else {
     ...
     * @return Matrix
     */
    public function multiMatrix(Matrix $matrix)
    {
        ...
        if ($dimensionsThis['cols'] != $dimensionsMatrix['rows']) {
            throw new Exception\RuntimeException("Number of columns in first matrix is not equal to number of rows in second matrix.");
        } else {
            $result = [[]];
            foreach ($this->matrix as $rowKey => $rowData) {
                foreach ($rowData as $colKey => $colValue) {
